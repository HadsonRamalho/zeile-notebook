...
use crate::{
    controllers::challenge::{
        api_add_test_case, api_create_challenge, api_delete_test_case, api_get_challenge,
        api_get_challenge_by_id, api_get_submission, api_leaderboard, api_list_challenges,
        api_list_my_submissions, api_list_test_cases, api_run_samples, api_set_reference,
...
pub async fn challenge_routes() -> OpenApiRouter<Arc<AppState>> {
    let routes = OpenApiRouter::<Arc<AppState>>::new()
        ...
        .route("/{id}", get(api_get_challenge_by_id).put(api_update_challenge))
        .route(
            "/{id}/test-cases",
