...
use super::dto::{FolderNameRequest, MoveFolderRequest};
use super::entity::{Folder, NewFolder};
...
pub async fn api_list_folders(
    ...
    let folders = repository::list_personal_folders(conn, user_id).await?;
...
pub async fn api_create_folder(
    ...
    let conn = &mut get_conn(&state.pool)
        .await
        .map_err(|e| ApiError::DatabaseConnection(e.1.0.to_string()))?;
    let folder = repository::create_folder(
        conn,
        &NewFolder {
            id: Uuid::new_v4(),
            name,
...
pub async fn api_rename_folder(
    ...
    let conn = &mut get_conn(&state.pool)
        .await
        .map_err(|e| ApiError::DatabaseConnection(e.1.0.to_string()))?;
    let folder = repository::get_folder(conn, folder_id).await?;
    if folder.user_id != Some(user_id) {
        return Err(ApiError::PermissionDenied("folder.manage".to_string()));
    }
    let updated = repository::rename_folder(conn, folder_id, &name).await?;
...
pub async fn api_update_folder_tags(
    ...
    let updated = repository::set_folder_tags(conn, folder_id, &tags).await?;
    Ok((StatusCode::OK, Json(updated)))
}
...
pub async fn api_delete_folder(
    ...
    repository::delete_folder(conn, folder_id).await?;
    Ok(StatusCode::OK)
}
...
pub async fn api_move_notebook_to_folder(
    ...
    repository::set_notebook_folder(conn, notebook_id, payload.folder_id).await?;
    Ok(StatusCode::OK)
}
...
pub async fn api_list_team_folders(
    ...
    let folders = repository::list_team_folders(conn, team_id).await?;
    Ok((StatusCode::OK, Json(folders)))
}
