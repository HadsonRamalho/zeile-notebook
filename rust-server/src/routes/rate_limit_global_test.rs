...
/// A `tokio::sync::Mutex` is used (not `std::sync::Mutex`) because the guard is held
/// across `.await` points in the async tests.
static OFF_VAR_LOCK: Mutex<()> = Mutex::const_new(());
...
fn the_global_ceiling_can_be_turned_off_for_load_testing() {
    let _guard = OFF_VAR_LOCK.blocking_lock();
...
async fn anonymous_traffic_has_a_per_origin_ceiling() {
    let _guard = OFF_VAR_LOCK.lock().await;
