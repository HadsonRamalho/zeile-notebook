...
escrita na etapa 17 do [plano de execução](plano-execucao.md), é a referência a citar em review
a partir de agora.

**Regra de precedência:** a regra documentada vence o padrão do arquivo vizinho. Se o código
em volta viola uma regra, siga a regra — não imite a violação. Um mau exemplo presente no
contexto não autoriza reproduzi-lo.

Questões ainda abertas ficam no [plano de execução](plano-execucao.md); a análise da branch
desktop está em [desktop-tauri.md](desktop-tauri.md).
...
| **Q12** | **Proibir referência a PR/issue/commit** em comentário (o git guarda) **e exigir link permanente** ao citar doc externo (DOI, número de RFC, URL arquivada). **ADR conta como referência legítima** — é versionada e perene, ao contrário de PR. | Relevante porque o Zeile comenta comportamento de lib externa: Automerge, perfect-freehand, Excalidraw, sql.js, Pyodide, Typst. Resolvido ao escrever [comment-guide.md](architecture/comment-guide.md). |
...
| **Q47 + Q48** | **Três camadas + DTO separado da entidade.** `controllers/` fino (extrai, chama, mapeia) · regra de negócio e autorização numa camada de serviço · acesso a banco isolado · e request/response DTO em arquivo próprio, separado do struct Diesel. **Raiz por módulo**: `domain/<nome>/{controller,service,repository,dto,entity}.rs`, não por camada. | Dissolve `models/notebook.rs` (1175 linhas) **por responsabilidade**, não por tamanho. O DTO separado é exatamente a fronteira que o gerador do Q28 precisa enxergar. Q4 exige cobertura antes — e `models/notebook.rs` já está na primeira leva do Q74. ADR retroativa: [0004-rust-module-organization](decisions/0004-rust-module-organization.md). |
...
| **Q89** | **Status de `permissions-design.md` corrigido para "implementado"** (fases 1–6 da seção 11 concluídas; fase 7, confidencial, deliberadamente adiada). A seção 3 ("Decisões batidas") virou ADR retroativa. | [0003-permission-model](decisions/0003-permission-model.md). |

### Fronteira entre `features/` (Q20) e as pastas de topo (Q24)

As duas decisões se sobrepõem em um ponto; a linha divisória, redigida em
[frontend-rules.md](architecture/frontend-rules.md) e com ADR retroativa em
[0005-features-boundary](decisions/0005-features-boundary.md):
...
| # | Decisão | Consequência |
|---|---|---|
| **Q108** 🔴 | **Revoga Q10. Comentários passam de pt-BR para en-US**, junto com identificadores, mensagens de log e `errorCode` (que já eram en-US). Motivo: a barreira à contribuição externa que Q10 aceitava como custo deixou de ser aceitável — o objetivo agora é justamente facilitar contribuição de fora. | Os ~205 comentários pt-BR do código deixam de estar conformes (ver Q94) e viram dívida a resolver, não consequência aceita. Tradução é feita arquivo por arquivo, minimizando comentário que só repete o nome do identificador (Q11 continua valendo: comentário só se justifica pelas 6 categorias — implementação de padrão externo, trade-off medido, invariante não local, violação intencional, regex/bitwise não trivial, decisão de UX). Identificadores que ainda estiverem em pt-BR (variável, função, arquivo, tipo) são levantados e renomeados no mesmo esforço. `components/vendor/*` continua isento (Q21). |

## Padronização do tratamento de erros no frontend
